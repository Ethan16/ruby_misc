...
def chg_conf(conf_plist,conf_py,password,config_dir,inj)
  #begin
  #改配置文件权限
  run_cmd="sudo chmod -R 777 /emm/bin/;cp -v -f %s %s;cp -v -f %s %s;%s restart" % [conf_py,config_dir,conf_plist,config_dir,inj]
  #exec run_sudo
  exp_run_cmd=Expectr.new(run_cmd,flush_buffer: true)
  exp_run_cmd.expect("Enter password:",false) do
    exp_run_cmd.puts(password)
    exp_run_cmd.interact!(blocking: false)
  end
=begin
  #覆盖配置文件
  ...
  exp_copy_conf.expect("Enter password:",false) do
  ...
  exp_restart_inj.expect("Enter password:",false) do
...
if (mac_emmi_path && (mac_emmi_path.include? mac_emmi_cur_version))
  ...
  puts "Now i will change the configuration."
  ...
  puts "Please make sure the package has been mapped!"
end
